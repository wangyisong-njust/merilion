...
def main(args):
    ...
    if args.block_wise:
        ...
        kwargs = {
            ...
            "consecutive_groups": {
                layer.self_attn.k_proj: layer.self_attn.head_dim for layer in model.model.text_model.layers + model.model.vision_model.encoder.layers
            },
            "customized_pruners": {
            ...
            "root_instances": [model.model.text_model.layers[i].self_attn.k_proj for i in range(args.block_attention_layer_start, args.block_attention_layer_end)] +
                              [model.model.text_model.layers[i].mlp.gate_proj for i in range(args.block_mlp_layer_start, args.block_mlp_layer_end)],
                            #   + [model.model.vision_model.encoder.layers[i].self_attn.k_proj for i in range(args.block_attention_layer_start, min(args.block_attention_layer_end, len(model.model.vision_model.encoder.layers)))]+
                            #   [model.model.vision_model.encoder.layers[i].mlp.fc1 for i in range(args.block_attention_layer_start, min(args.block_mlp_layer_end, len(model.model.vision_model.encoder.layers)))],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_model_path', type=str, default="./vlm_checkpoints/SmolVLM", help=' ')
