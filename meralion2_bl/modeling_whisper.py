...
class WhisperEncoderLayer(nn.Module):
    def __init__(self, config: WhisperConfig, layer_idx: int):
        ...
        midblock_ratio = getattr(config, "midblock_ratio", 1.0)
        self.ratio = midblock_ratio if self.midblock_start <= layer_idx < self.midblock_end else 1.0

        self.encoder_ffn_dim = int(config.encoder_ffn_dim * self.ratio)

        self.self_attn = WHISPER_ATTENTION_CLASSES[config._attn_implementation](
            embed_dim=self.embed_dim,
            num_heads=int(config.encoder_attention_heads * self.ratio),
    ...
    def resize_to_match(self, state_dict, prefix):
        q_weight_name = f"{prefix}.self_attn.q_proj.weight"
        if q_weight_name in state_dict:
            new_q_out = state_dict[q_weight_name].shape[0]
            if new_q_out != self.self_attn.q_proj.out_features:
                print(f"Resizing {prefix} Attn: {self.self_attn.q_proj.out_features}->{new_q_out}")
                self.self_attn.q_proj = nn.Linear(self.self_attn.q_proj.in_features, new_q_out, bias=True)
                self.self_attn.k_proj = nn.Linear(self.self_attn.k_proj.in_features, new_q_out, bias=False)
                self.self_attn.v_proj = nn.Linear(self.self_attn.v_proj.in_features, new_q_out, bias=True)
                self.self_attn.out_proj = nn.Linear(new_q_out, self.self_attn.out_proj.out_features, bias=True)
                self.self_attn.num_heads = new_q_out // (self.self_attn.head_dim)
                self.self_attn.embed_dim = new_q_out
        
        fc1_weight_name = f"{prefix}.fc1.weight"
        if fc1_weight_name in state_dict:
            new_fc1_out = state_dict[fc1_weight_name].shape[0]
            if new_fc1_out != self.fc1.out_features:
                print(f"Resizing {prefix} FC: {self.fc1.out_features}->{new_fc1_out}")
                self.encoder_ffn_dim = new_fc1_out
                self.fc1 = nn.Linear(self.fc1.in_features, new_fc1_out)
                self.fc2 = nn.Linear(new_fc1_out, self.fc2.out_features)
...
class WhisperEncoder(WhisperPreTrainedModel):
    ...
    def __init__(self, config: WhisperConfig):
        ...
        self.layers = nn.ModuleList([WhisperEncoderLayer(config, idx) for idx in range(config.encoder_layers)])
    ...
    def resize_to_match(self, state_dict, prefix):
        for i, layer in enumerate(self.layers):
            layer.resize_to_match(state_dict, f"{prefix}.layers.{i}")
