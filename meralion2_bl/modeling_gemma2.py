...
class Gemma2MLP(nn.Module):
    ...
    def resize_to_match(self, state_dict, prefix):
        gate_weight_name = f"{prefix}.gate_proj.weight"
        if gate_weight_name in state_dict:
            new_inter = state_dict[gate_weight_name].shape[0]
            if new_inter != self.gate_proj.out_features:
                print(f"Resizing {prefix} MLP: {self.gate_proj.out_features}->{new_inter}")
                self.intermediate_size = new_inter
                self.gate_proj = nn.Linear(self.gate_proj.in_features, new_inter, bias=False)
                self.up_proj = nn.Linear(self.up_proj.in_features, new_inter, bias=False)
                self.down_proj = nn.Linear(new_inter, self.down_proj.out_features, bias=False)
...
class Gemma2Attention(nn.Module):
    ...
    def resize_to_match(self, state_dict, prefix):
        q_weight_name = f"{prefix}.q_proj.weight"
        if q_weight_name in state_dict:
            new_q_out = state_dict[q_weight_name].shape[0]
            new_k_out = state_dict[f"{prefix}.k_proj.weight"].shape[0]
            
            if new_q_out != self.q_proj.out_features or new_k_out != self.k_proj.out_features:
                print(f"Resizing {prefix} heads: Q {self.q_proj.out_features}->{new_q_out}, KV {self.k_proj.out_features}->{new_k_out}")
                self.num_attention_heads = new_q_out // self.head_dim
                self.num_key_value_heads = new_k_out // self.head_dim
                self.q_proj = nn.Linear(self.q_proj.in_features, new_q_out, bias=self.q_proj.bias is not None)
                self.k_proj = nn.Linear(self.k_proj.in_features, new_k_out, bias=self.k_proj.bias is not None)
                self.v_proj = nn.Linear(self.v_proj.in_features, new_k_out, bias=self.v_proj.bias is not None)
                self.o_proj = nn.Linear(new_q_out, self.o_proj.out_features, bias=self.o_proj.bias is not None)
                self.num_key_value_groups = self.num_attention_heads // self.num_key_value_heads
...
class Gemma2DecoderLayer(nn.Module):
    ...
    def resize_to_match(self, state_dict, prefix):
        self.self_attn.resize_to_match(state_dict, f"{prefix}.self_attn")
        self.mlp.resize_to_match(state_dict, f"{prefix}.mlp")
...
class Gemma2ForCausalLM(Gemma2PreTrainedModel, GenerationMixin):
    ...
    def resize_to_match(self, state_dict, prefix):
        self.model.resize_to_match(state_dict, f"{prefix}.model")
