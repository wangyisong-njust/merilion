...
class Dataset(object):
    ...
    def load_dataset(self):
        ...
        if   self.dataset_name == 'cn_college_listen_mcq_test': 
            ...
            try:
                ds = load_from_disk("/home/jinchao/runtao/meralion_datasets/ASR/IMDA_PART1_mono_en_30_ASR")
                if 'context' in ds.column_names and 'audio' not in ds.column_names:
                    ds = ds.map(lambda x: {"audio": x["context"]["audio"], "instruction": x["instruction"]["text"], "reference": x["answer"]["text"]})
                self.raw_data = ds
            except:
            ...
            try:
                # Use PART3 as a fallback if PART2 is not available locally
                ds = load_from_disk("/home/jinchao/runtao/meralion_datasets/ASR/IMDA_PART3_conv_en_30_ASR")
                if 'context' in ds.column_names and 'audio' not in ds.column_names:
                    ds = ds.map(lambda x: {"audio": x["context"]["audio"], "instruction": x["instruction"]["text"], "reference": x["answer"]["text"]})
                self.raw_data = ds
            except:
    ...
    def data_format(self):
        ...
        if self.dataset_name == 'cn_college_listen_mcq_test': 
            ...
            from audiobench.dataset_src.imda_part1_asr_test import imda_part1_asr_test_dataset
            self.dataset_processor = imda_part1_asr_test_dataset(self.raw_data, self.number_of_samples)

        elif self.dataset_name == 'imda_part2_asr_test':
            from audiobench.dataset_src.imda_part2_asr_test import imda_part2_asr_test_dataset
            self.dataset_processor = imda_part2_asr_test_dataset(self.raw_data, self.number_of_samples)

        elif self.dataset_name == 'imda_part3_30s_asr_test':
            from audiobench.dataset_src.imda_part3_30s_asr_test import imda_part3_30s_asr_test_dataset
            self.dataset_processor = imda_part3_30s_asr_test_dataset(self.raw_data, self.number_of_samples)

        elif self.dataset_name == 'imda_part4_30s_asr_test':
            from audiobench.dataset_src.imda_part4_30s_asr_test import imda_part4_30s_asr_test_dataset
            self.dataset_processor = imda_part4_30s_asr_test_dataset(self.raw_data, self.number_of_samples)

        elif self.dataset_name == 'imda_part5_30s_asr_test': 
            from audiobench.imda_part5_30s_asr_test import imda_part5_30s_asr_test_dataset
            self.dataset_processor = imda_part5_30s_asr_test_dataset(self.raw_data, self.number_of_samples)

        elif self.dataset_name == 'imda_part6_30s_asr_test': 
            from audiobench.imda_part6_30s_asr_test import imda_part6_30s_asr_test_dataset
