...
def transcribe_gpu_draft_spec(
    ...
    with torch.inference_mode():
        ...
        while len(generated_ids) < max_new_tokens:
            ...
            for _di in range(_gamma_eff):
                ...
                if _diag_first_draft and _di == 0:
                    ...
                    print(f"  [DIAG] draft logits step0: "
                          f"nan={torch.isnan(_d0).any().item()} "
                          f"inf={torch.isinf(_d0).any().item()} "
            ...
            # Single CPU sync for the whole batch of proposed tokens.
            if _d_toks_gpu:
                draft_tokens = torch.cat(_d_toks_gpu, dim=1)[0].tolist()
            else:
                draft_tokens = []

            # Truncate at the first EOS so the verifier doesn't waste slots
            # past an early-stopping token.
            for _i, _t in enumerate(draft_tokens):
                if _t in eos_ids:
                    draft_tokens = draft_tokens[: _i + 1]
                    break
            K = len(draft_tokens)
