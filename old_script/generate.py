...
def main(args):
    ...
    # Terminal-based input/output loop
    print("Enter input text (type 'exit' to quit):")
    while True:
        user_input = input("Input: ")
        if user_input.lower() == "exit":
            break
        output = evaluate(input=user_input)
...
'''
    ...
    ).queue().launch(server_name="0.0.0.0", server_port=7860, share=False) #share=args.share_gradio)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--base_model', type=str, default="baffo32/decapoda-research-llama-7B-hf", help='base model name')
    parser.add_argument('--model_type', type=str, required=True, help='choose from pretrain, pruneLLM, tune_prune_LLM')
    parser.add_argument('--ckpt', type=str, default=None)
    parser.add_argument('--lora_ckpt', type=str, default=None)
