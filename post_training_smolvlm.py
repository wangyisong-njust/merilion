...
def main(args):
    ...
    textvqa_train_dataset = textvqa_train_dataset.filter(lambda example: "fruit".lower() in example["question"].lower())
    textvqa_eval_dataset = textvqa_eval_dataset.filter(lambda example: "fruit".lower() in example["question"].lower())   
    textvqa_test_dataset = textvqa_test_dataset.filter(lambda example: "fruit".lower() in example["question"].lower())

    print("filtered out textvqa dataset:", len(textvqa_train_dataset), len(textvqa_eval_dataset), len(textvqa_test_dataset))
        
    # sharegpt_dataset = load_dataset("OpenGVLab/ShareGPT-4o", 'image_caption', split="images").select(range(5000)).train_test_split(
    #         test_size=args.val_set_size, shuffle=True, seed=42
    #     )
    
    # sharegpt_train_dataset = sharegpt_dataset["train"]
    # sharegpt_eval_dataset = sharegpt_dataset["test"]
    ...
    # llava
    CocoDetection(root=args.coco_data_path+"/2017", annFile=args.coco_data_path+"/annotations/instances_train2017.json")
    dataset = load_dataset(args.llava_data_path, data_files="llava_instruct_150k.json", split="train")
    ...
    if args.cache_dataset and os.path.exists('datasets/cache/{}.bin'.format(args.data_path)):
        ...
        # llava_wild_dataset = build_llava_wild_data()
        train_dataset = [format_llava_data(sample, img_root=args.coco_data_path+"/train2017") for sample in llava150K_train_dataset] #+ llava_wild_dataset * 34
        eval_dataset = [format_llava_data(sample, img_root=args.coco_data_path+"/train2017") for sample in llava150K_eval_dataset][:1000] #+ llava_wild_dataset

        train_dataset += [format_textvqa_data(sample) for sample in textvqa_train_dataset]
        eval_dataset += [format_textvqa_data(sample) for sample in textvqa_eval_dataset]
        test_dataset = eval_dataset + [format_textvqa_data(sample) for sample in textvqa_test_dataset]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--coco_data_path', type=str, default='/home/kaixin/datasets/coco', help='coco dataset path')
    parser.add_argument('--llava_data_path', type=str, default='/home/kaixin/datasets/LLaVA-Instruct-150K', help='LLaVA-Instruct-150K dataset path')
