...
```bash
ENV=/home/jinchao/miniconda3/envs/audiobench_quant_vLLMv0
...
# 2. 设 conda env 的 nvcc + CUDA home（系统 /usr/local/cuda-12.8 不存在）
export PATH=$ENV/bin:$PATH
export CUDA_HOME=$ENV

# 3. 补 CUDA dev headers
...
# 5. 设运行时 LD_LIBRARY_PATH（两个库都要）
export LD_LIBRARY_PATH=$($PYTHON_PATH -c "import torch,os;print(os.path.dirname(torch.__file__))")/lib:$ENV/lib:$LD_LIBRARY_PATH
...
注意：LD_LIBRARY_PATH 只在当前 shell session 生效，bench 脚本里需要在开头加这两行 export。
