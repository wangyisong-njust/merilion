...
### 15.4 环境重建（auto-gptq CUDA 扩展）

重建 `audiobench_quant_vLLMv0` 环境里的 auto-gptq CUDA 扩展完整步骤：

```bash
WORKDIR=/home/jinchao/runtao/LLM-Pruner
# 主推理环境
PYTHON_PATH=/home/jinchao/miniconda3/envs/audiobench_quant_vLLMv0/bin/python

# 1. 诊断：确认 CUDA ext 没编进去
$PYTHON_PATH $WORKDIR/probe_marlin_env.py   # 应看到 "CUDA extension not installed."

# 2. 设编译环境（encmamba 提供 nvcc/CUDA_HOME/libcudart，主环境提供 torch）
#    setup_cuda_includes.sh 同时处理 CPATH + encmamba PATH/CUDA_HOME/LD_LIBRARY_PATH
source $WORKDIR/setup_cuda_includes.sh
# torch/lib 也要在 LD_LIBRARY_PATH（libc10.so 等）
export LD_LIBRARY_PATH=$($PYTHON_PATH -c "import torch,os;print(os.path.dirname(torch.__file__))")/lib:$LD_LIBRARY_PATH

# 3. clone + patch + 编译（用主环境的 python 装进 audiobench_quant_vLLMv0）
git clone --depth=1 https://github.com/AutoGPTQ/AutoGPTQ.git /tmp/AutoGPTQ
$PYTHON_PATH $WORKDIR/patch_autogptq_marlin_only.py
cd /tmp/AutoGPTQ && rm -rf build dist *.egg-info
$PYTHON_PATH setup.py bdist_wheel 2>&1 | tee /tmp/autogptq_build.log
$PYTHON_PATH -m pip install --no-deps --force-reinstall dist/auto_gptq*.whl

# 4. 验证
$PYTHON_PATH -c "import exllama_kernels; print('exllama OK:', exllama_kernels.__file__)"
$PYTHON_PATH -c "import exllamav2_kernels; print('exllamav2 OK:', exllamav2_kernels.__file__)"
$PYTHON_PATH $WORKDIR/probe_marlin_env.py
```

注意：LD_LIBRARY_PATH 只在当前 shell session 生效，bench 脚本开头需要重新 source。
...
| 编译后 `import exllama_kernels` 报 `libcudart.so.12: cannot open shared object file` | conda env 的 CUDA runtime lib 不在 LD_LIBRARY_PATH | `export LD_LIBRARY_PATH=/home/jinchao/miniconda3/envs/audiobench_quant_vLLMv0/lib:$LD_LIBRARY_PATH`（需同时保留 torch/lib，见上一行）|
| 1 次 warmup 不够 — bench latency 偏高 ~20% | exllama kernel cache 需要 2-3 次 forward 才稳定（实测 warmup1=1.38s, warmup2=0.73s, warmup3=0.69s）| `infer_gpu_eagle.py` 改为 3 次 warmup pass，第一个 timed sample 才在 steady state |
