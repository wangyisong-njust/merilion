...
PYTHON_PATH=/home/jinchao/miniconda3/envs/encmamba/bin/python

# 1. 诊断：确认 CUDA ext 没编进去
$PYTHON_PATH $WORKDIR/probe_marlin_env.py   # 应看到 "CUDA extension not installed."

# 2. 设 encmamba nvcc + CUDA_HOME + CPATH + LD_LIBRARY_PATH（一步到位）
...
# 4. 验证（LD_LIBRARY_PATH 已由 setup_cuda_includes.sh 设好）
...
注意：LD_LIBRARY_PATH 只在当前 shell session 生效，bench 脚本开头需要 `source setup_cuda_includes.sh` 或手动 export。
