...
from datasets import Dataset as Dataset_datasets
...
def main(args):
    ...
    config = LoraConfig(
        ...
        task_type="CAUSAL_LM"
        # task_type="SEQ_2_SEQ_LM", # ERROR: MERaLiON2ForConditionalGeneration.forward() got an unexpected keyword argument 'decoder_input_ids'
    )
    ...
    DATASET_ID_TRAIN_30s = "imda_part3_30s_asr_test"
    DATASET_ID_EVAL_30s = "imda_part4_30s_asr_test"
    ...
    # mix_dataset = False
    if mix_dataset:
        train_dataset_small = Dataset(DATASET_ID_TRAIN, nsamples).raw_data
        train_dataset_30s = Dataset(DATASET_ID_TRAIN_30s, nsamples).raw_data
        # train_dataset = concatenate_datasets([train_dataset_small, train_dataset_30s]).shuffle(seed=42)
        train_dataset = interleave_datasets(
            [train_dataset_small, train_dataset_30s],
    ...
    eval_dataset = Dataset(DATASET_ID_EVAL, nsamples).raw_data.shuffle(seed=42).select(range(200))
    ...
    class DataCollatorSpeechSeq2SeqWithPadding:
        ...
        def __call__(self, features):
            ...
            for f in features:
                ...
                # chat_texts.append(build_chat_prompt(self.processor, instruction, answer))
    ...
    class Meralion2SFTCollator:
        ...
        def __call__(self, features):
            ...
            # Build labels: copy input_ids then mask the prompt portion + pads
            labels = batch["input_ids"].clone()

            # Compute prompt lengths per sample (tokenize user_texts only)
            user_tok = self.tokenizer(
                user_texts,
                padding=True,            # padded, but we use attention_mask to get true lengths
                truncation=True, # the 2 lines are optioanl ?
                max_length=1024, 
                return_tensors="pt",
                add_special_tokens=False,
            )
            prompt_lens = user_tok["attention_mask"].sum(dim=1)  # (B,)
    ...
    # data_collator = Meralion2SFTCollator(processor)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lora_dropout', type=float, default=0.05, help='lora dropout') # ori 0.05
