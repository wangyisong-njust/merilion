...
class TextVQAAccuracyEvaluator:
    def __init__(self, match_constraint = False):
        self.answer_processor = EvalAIAnswerProcessor()
        self.match_constraint = match_constraint
    ...
    def _compute_answer_scores(self, raw_answers):
        ...
        for unique_answer in unique_answers:
            ...
            # set score to 1 if got >= 5 matches
            if self.match_constraint:
                matching_answers = [
                    item for item in gt_answers if item[1] == unique_answer
                ]
                if len(matching_answers) >= 5:
                    unique_answer_scores[unique_answer] = 1.0
                else:
                    unique_answer_scores[unique_answer] = 0.0
...
def eval_single(annotation_file, result_file):
    ...
    for result in results:
        ...
        pred_list.append({
            "pred_answer": result['text'][0][result['text'][0].index('Assistant:') + len('Assistant:'):].strip(),
            "gt_answers": annotation['answers'],
        })


    evaluator = TextVQAAccuracyEvaluator(False)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = load_image(Image.open(os.path.join(self.image_folder, image_file)))
        messages = [
        {
            "role": "user",
            "content": [
...
def eval_model(model, processor, 
    ...
    data_loader = create_data_loader(questions, image_folder, processor)
    ...
    eval_single(
        ...
        result_file=os.path.expanduser(answers_file)
    )
