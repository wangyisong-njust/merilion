#!/bin/bash
# ============================================================
# AutoAWQ4 speculative decoding benchmark (n-gram spec)
# Compares no-spec vs n-gram +spec on GPU for:
#   - Original MERaLiON-2-3B  (BF16 baseline + AutoAWQ4)
#   - Pruned mid3-23           (BF16 baseline + AutoAWQ4)
# ============================================================
...
MID3_23_AWQ="${WORKDIR}/MERaLiON-2-3B-mid3-23-AutoAWQ4"
...
echo ""
echo "========================================"
echo "  Original MERaLiON-2-3B  AutoAWQ4"
echo "========================================"

echo ""
echo "--- AutoAWQ4 no-spec ---"
...
echo ""
echo "--- AutoAWQ4 +spec γ=${GAMMA} ---"
run_if_missing "gpu_autoawq4_original_spec_g${GAMMA}.json" \
    --model "$ORIGINAL_AWQ" --dataset "$DATASET" \
    --num_samples "$NUM_SAMPLES" --quant autoawq4 \
    --speculative --gamma "$GAMMA" --corpus "$CORPUS" || exit 1
...
echo ""
echo "========================================"
echo "  Pruned mid3-23  AutoAWQ4"
echo "========================================"

echo ""
echo "--- AutoAWQ4 no-spec ---"
run_if_missing "gpu_autoawq4_mid3-23_nospec.json" \
    --model "$MID3_23_AWQ" --dataset "$DATASET" \
    --num_samples "$NUM_SAMPLES" --quant autoawq4 || exit 1

echo ""
echo "--- AutoAWQ4 +spec γ=${GAMMA} ---"
run_if_missing "gpu_autoawq4_mid3-23_spec_g${GAMMA}.json" \
    --model "$MID3_23_AWQ" --dataset "$DATASET" \
    --num_samples "$NUM_SAMPLES" --quant autoawq4 \
    --speculative --gamma "$GAMMA" --corpus "$CORPUS" || exit 1
...
rows = [
    ...
    (f"Original  AutoAWQ4  +spec γ={G}",  f"gpu_autoawq4_original_spec_g{G}.json",      True),
    ("mid3-23   BF16      no-spec",       "gpu_bf16_mid3-23_nospec.json",               False),
    (f"mid3-23   BF16      +spec γ={G}",  f"gpu_bf16_mid3-23_spec_g{G}.json",           True),
    ("mid3-23   AutoAWQ4  no-spec",       "gpu_autoawq4_mid3-23_nospec.json",            False),
    (f"mid3-23   AutoAWQ4  +spec γ={G}",  f"gpu_autoawq4_mid3-23_spec_g{G}.json",       True),
]

ref_lat = None
hdr = f"  {'Config':<38} {'Lat(s)':>7} {'Speedup':>8} {'tok/s':>7} {'WER%':>6} {'VRAM(GB)':>9} {'AccRate':>8}"
print(hdr)
print("  " + "-" * (len(hdr) - 2))
...
for label, path, is_spec in rows:
    ...
    print(f"  {label:<38} {lat:7.2f}  {ratio:7.2f}x  {tps:7.1f}  {wer:6.2f}  {vram_s:>9}  {acc_s:>8}{marker}")
PYEOF
